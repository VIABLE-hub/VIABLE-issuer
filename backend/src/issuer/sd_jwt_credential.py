...
def generate_sd_jwt_credential(auth_header, private_key, issuer_did, issuer_kid):
    if not auth_header:
    ...
    credential_data_entry = VC_Offer.query.filter_by(
    ...
    sd_specification = {
        "vc": {
            "credentialSubject": {
                 key: True for key in user_claims.keys()
            }
        }
    }
