    ...
    if id_jwt:
        try:
            ...
            authorization_code_entry = VC_AuthorizationCode.query.filter_by(
                client_id=iss, used=False).first()

            if authorization_code_entry:
                logger.info(f"Found existing entry for client_id {iss}, updating authorization code")
                ...
                try:
                    db.session.commit()
                    logger.info(f"Successfully updated authorization code for client_id: {iss}")
                    logger.info(f"Updated entry: {authorization_code_entry}")
                except Exception as e:
                    logger.error(f"Failed to update authorization code: {e}")
                    db.session.rollback()
                    return jsonify({"error": "Failed to update authorization code"}), 500
                    
            else:
                logger.info(f"No unused entry found for client_id {iss}, creating new one")
                ...
                logger.info(f"All entries for client_id {iss}: {[str(entry) for entry in all_entries]}")
                
                # Create a new entry - this should ideally not happen if authorization flow is correct
                new_entry = VC_AuthorizationCode(
                    client_id=iss,
                    auth_code=authorization_code,
                    code_challenge=request.json.get("code_challenge") if request.json else None,
                    issuer_state=request.json.get("issuer_state") if request.json else state,
                    used=False
                )
                ...
                try:
                    ...
                    db.session.rollback()
                    return jsonify({"error": "Failed to create authorization code"}), 500
