...
from .field_extractor import get_field_value
from .constants import BBS_METADATA_FIELDS
...
def handle_oversized_field(field_key, value, max_length=10000):
    ...
    if isinstance(value, str) and len(value) > max_length:
        logger.warning(f"Feld '{field_key}' ist zu groß ({len(value)} Zeichen), wird gekürzt")
    ...
    try:
        ...
        for key, value in values.items():
            if isinstance(value, dict):
                logger.info(f"Field '{key}': [nested object with keys: {list(value.keys())}]")
        ...
        disclosed_values_raw = {k: v for k, v in values.items() if k not in excluded_bbs_fields}
        ...
        logger.info(f"🔹 Flattened disclosed values: {len(disclosed_values_flat)} fields")
        logger.debug(f"🔹 Flattened field names: {sorted(disclosed_values_flat.keys())}")
        ...
        for key in sorted(disclosed_values_flat.keys()):
            ...
            message = json.dumps({key: value}, ensure_ascii=False, separators=(',', ':'))
            disclosed_messages.append(message)
            logger.debug(f"🔹 Created message: {message}")
        
        logger.info(f"🔹 Created {len(disclosed_messages)} disclosed messages for BBS+ verification")
        ...
        try:
            ...
            if result == "true":
                ...
                if result == "matchin messages to presentation failed":
                    ...
                    logger.error("❌ This indicates the messages format doesn't match what was used during proof generation")
                    logger.error(f"❌ Disclosed messages count: {len(disclosed_messages)}")
